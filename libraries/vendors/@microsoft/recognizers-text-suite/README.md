...
`npm install botbuilder-recognizers-text-suite`

Or, if you prefer to use a single type of recognizer:

* Get **only** the numbers Recognizer's features:
`npm install botbuilder-recognizers-text-number`

* Get **only** the numbers with units Recognizer's features:
`npm install botbuilder-recognizers-text-number-with-unit`

* Get **only** the date and time Recognizer's features:
`npm install botbuilder-recognizers-text-date-time`

* Get **only** the sequence Recognizer's features:
`npm install botbuilder-recognizers-text-sequence`

* Get **only** the choice Recognizer's features:
`npm install botbuilder-recognizers-text-choice`
...
````JavaScript
var Recognizers = require('botbuilder-recognizers-text-suite');
var NumberRecognizers = require('botbuilder-recognizers-text-number');
var NumberWithUnitRecognizers = require('botbuilder-recognizers-text-number-with-unit');
var DateTimeRecognizers = require('botbuilder-recognizers-text-date-time');
var SequenceRecognizers = require('botbuilder-recognizers-text-sequence');
var ChoiceRecognizers = require('botbuilder-recognizers-text-choice');
````
