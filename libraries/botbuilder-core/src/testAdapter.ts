...
import { BotFrameworkAuthentication, BotFrameworkAuthenticationFactory, UserTokenClient } from 'botframework-connector';
import { tokenExchangeOperationName } from './signInConstants';
...
export class TestAdapter extends BotAdapter implements ExtendedUserTokenProvider {
    ...
    private cloudAdapter: TestCloudAdapter;
    ...
    async processActivity(
        ...
        const context = this.createContext(request, isCloudAdapter);
        if (callback) {
     ...
     */
    protected createContext(request: Partial<Activity>, isCloudAdapter = false): TurnContext {
        if (isCloudAdapter) {
            this.cloudAdapter = new TestCloudAdapter(this.exchangeableTokens, request)
            return this.cloudAdapter.context;
        }
...
class TestCloudAdapter extends CloudAdapterBase {
    ...
    constructor(
        ...
        this.context = new TurnContext(this, activity);
        const userTokenClient = new UserTokenClientKey(exchangeableTokens);
        this.context.turnState.set(this.UserTokenClientKey, userTokenClient);
    }
}

class UserTokenClientKey extends UserTokenClient {
    private exchangeableTokens: { [key: string]: ExchangeableToken } = {};
    private readonly ExceptionExpected: string = 'ExceptionExpected';

    constructor(exchangeableTokens) {
        super();
        this.exchangeableTokens = exchangeableTokens;
    }
                ...
                return new Promise<void>((resolve: any, reject: any): void => {
                    ...
                    function waitForActivity(): void {
                        ...
                        if (current - start > <number>timeout) {
                            ...
                            reject(
                                new Error(
                                    `TestAdapter.assertReply(${expecting}): ${description} Timed out after ${current - start
                                    }ms.`
                                )
                            );
